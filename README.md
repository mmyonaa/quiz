...
- **실기 채점**: `src/lib/practical-grade.ts` — 표기 요동(대소문자·띄어쓰기·전각·낱말 하이픈)을 정규화로 흡수하고, 뜻이 같은 다른 낱말은 문항이 허용 표기 목록으로 나열한다. 그래도 새는 표기는 사용자가 "이 표기도 정답 처리"로 로컬에 덧붙인다.
- **문제 원칙**: 기출 복제가 아닌 자체 제작 문항. 전 문항 하/중/상 난이도 라벨. 코드 출력 문항은 실제 실행으로 정답을 검산한다.
- **개념 주제**: `src/data/topic-notes.json`이 주제의 단일 정의 — 영역·제목·자족적 도입부·연결할 블로그 글(선택). 필기와 실기가 같은 주제 집합을 공유하며, 문항은 `topic`으로 주제에 속하고, 없는 주제를 쓰면 빌드가 실패한다. 주제는 블로그와 분리돼 있어 글이 없어도 도입부로 완결된다.
