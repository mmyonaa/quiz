...
import { AREAS, SUBJECTS, TOPICS } from "../content.config";
...
export function topicsOfSubject(subject: (typeof SUBJECTS)[number]) {
  return Object.entries(TOPICS)
...
export function areasOfSubject(subject: (typeof SUBJECTS)[number]) {
  return AREAS.filter((a) => Object.values(TOPICS).some((t) => t.subject === subject && t.area === a));
}
